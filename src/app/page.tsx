...
export default function LoginPage() {
  ...
  const [signingIn, setSigningIn] = useState<"visitor" | "admin" | null>(null);
  ...
  const handleVisitorLogin = async () => {
    ...
    try {
      ...
      if (data.isBlocked) {
        await signOut(auth);
        setError("You are not allowed to use the library.");
        return;
      }
  ...
  const handleAdminLogin = async () => {
    ...
    try {
      ...
      if (!userSnap.exists()) {
        await signOut(auth);
        setError("Admin account not found. Contact administrator.");
        return;
      }
        ...
        <div className="w-full md:w-1/2 bg-white p-6 md:p-10 space-y-8">
          ...
          <div className="space-y-3">
            ...
            >
              ...
              {signingIn === "visitor" ? "Signing in..." : "Sign in as Visitor"}
            </button>

            <button
              onClick={handleAdminLogin}
              disabled={signingIn !== null}
              className="w-full bg-[#006600] text-white rounded-xl py-3.5 flex items-center justify-center gap-3 hover:bg-green-800 transition font-bold text-xs uppercase tracking-wide"
            >
              <ShieldCheck className="h-5 w-5" />
              {signingIn === "admin" ? "Signing in..." : "Sign in as Admin"}
            </button>
          </div>
